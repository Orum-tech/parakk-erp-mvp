...
class _StudentChatDetailScreenState extends State<StudentChatDetailScreen> {
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      ...
      _chatId = await _chatService.getOrCreateChatId(user.uid, widget.studentId);

      // Mark messages as read
      await _chatService.markMessagesAsRead(_chatId!, widget.studentId);
      ...
      return Scaffold(
        appBar: AppBar(
          title: Column(
            ...
            children: [
              Text(widget.studentName, style: const TextStyle(color: Colors.black, fontSize: 16)),
              Text("Roll: ${widget.rollNumber} - ${widget.studentClass}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
            ],
          ),
